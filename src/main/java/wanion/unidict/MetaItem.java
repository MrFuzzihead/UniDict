...
public final class MetaItem {
    ...
    public static int get(final ItemStack itemStack) {
        ...
        return id > 0
            ? item.getDamage(itemStack) == OreDictionary.WILDCARD_VALUE ? id : id | item.getDamage(itemStack) + 1 << 16
            : 0;
    }
    ...
    public static ItemStack toItemStack(final int metaItemKey) {
        return metaItemKey > 0
            ? new ItemStack(itemRegistry.getRaw(metaItemKey ^ (metaItemKey & 65536)), 0, metaItemKey >> 16)
            : null;
    }
    ...
    public static int getCumulative(final Object[] objects, final ResourceHandler resourceHandler) {
        ...
        for (final Object object : objects)
            if (object instanceof ItemStack) cumulativeKey += get(resourceHandler.getMainItemStack((ItemStack) object));
    ...
    public static int getCumulative(final ItemStack... itemStacks) {
        int cumulativeKey = 0;
        for (final ItemStack itemStack : itemStacks) cumulativeKey += get(itemStack);
        return cumulativeKey;
    }
    ...
    public static TIntList getList(@Nonnull final Object[] objects, @Nonnull final ResourceHandler resourceHandler) {
        final TIntList keys = new TIntArrayList();
        int bufKey;
        for (final Object object : objects) if (object instanceof ItemStack) {
            if ((bufKey = get(resourceHandler.getMainItemStack((ItemStack) object))) > 0) keys.add(bufKey);
        } else if (object instanceof List && !((List) object).isEmpty())
            if ((bufKey = get(((ItemStack) ((List) object).get(0)))) > 0) keys.add(bufKey);
        return keys;
    }
    ...
    public static TIntSet getSet(final Collection<Resource> resourceCollection, final long kind) {
        ...
        resourceCollection.stream()
            .filter(resource -> (resource.getChildren() & kind) > 0)
            .forEach(
                resource -> keys.addAll(
                    getList(
                        resource.getChild(kind)
                            .getEntries())));
        return keys;
    }
