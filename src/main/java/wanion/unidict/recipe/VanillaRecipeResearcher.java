...
public final class VanillaRecipeResearcher implements IRecipeResearcher<ShapedRecipes, ShapelessRecipes> {
    ...
    public int getShapelessRecipeKey(@Nonnull final IRecipe recipe, @Nonnull final ResourceHandler resourceHandler) {
        final TIntList recipeKeys = MetaItem
            .getList(((ShapelessRecipes) recipe).recipeItems.toArray(), resourceHandler);
        int recipeKey = 0;
        recipeKeys.sort();
        for (final TIntIterator recipeKeysIterator = recipeKeys.iterator(); recipeKeysIterator.hasNext();)
            recipeKey += 31 * recipeKeysIterator.next();
    ...
    public ShapedOreRecipe getNewShapedFromShapelessRecipe(@Nonnull final IRecipe recipe,
        ...
        for (int i = 0; i < recipeInputs.size(); i++)
            newRecipeInputs[i] = (bufferOreName = resourceHandler.getContainerName(recipeInputs.get(i))) != null
                ? bufferOreName
                : recipeInputs.get(i);
        return new ShapedOreRecipe(
            resourceHandler.getMainItemStack(recipe.getRecipeOutput()),
