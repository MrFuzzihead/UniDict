...
public final class IC2RecipeResearcher implements IRecipeResearcher<AdvRecipe, AdvShapelessRecipe> {
    ...
    public int getShapedRecipeKey(@Nonnull IRecipe recipe, @Nonnull final ResourceHandler resourceHandler) {
        ...
        for (final Object input : ((AdvRecipe) recipe).input) {
            if (input instanceof ItemStack)
                ...
                else if (obj instanceof IRecipeInput) recipeKeys.add(
                    MetaItem.get(
                        ((IRecipeInput) obj).getInputs()
                            .get(0)));
            } else if (input instanceof IRecipeInput) recipeKeys.add(
                MetaItem.get(
                    ((IRecipeInput) input).getInputs()
                        .get(0)));
        }
        recipeKeys.sort();
    ...
    public ShapedOreRecipe getNewShapedRecipe(@Nonnull IRecipe recipe, @Nonnull final ResourceHandler resourceHandler,
        @Nonnull final UniOreDictionary uniOreDictionary) {
        ...
        for (int i = 0; i < 9 && i < recipeInputs.length; i++) {
            if (recipeInputs[i] instanceof IRecipeInput) {
                ...
                newRecipeInputs[i] = bufferOreName != null ? bufferOreName
                    : (bufferOreName = uniOreDictionary.getName(inputs.get(0))) != null ? bufferOreName : inputs.get(0);
            } else {
                String bufferOreName = uniOreDictionary.getName(recipeInputs[i]);
                if (bufferOreName != null) newRecipeInputs[i] = bufferOreName;
    ...
    @Override
    @Nonnull
    public ShapedOreRecipe getNewShapedFromShapelessRecipe(@Nonnull IRecipe recipe,
        ...
        for (final Object recipeInput : ((AdvShapelessRecipe) recipe).input) {
            if (recipeInput instanceof List) {
                ...
                if (input instanceof IRecipeInput) {
                    ...
                    String bufferOreName = uniOreDictionary.getName(inputs);
                    newInputs.add(
                        bufferOreName != null ? bufferOreName
                            : (bufferOreName = uniOreDictionary.getName(inputs.get(0))) != null ? bufferOreName
                                : inputs.get(0));
                ...
                if (bufferOreName != null) newInputs.add(bufferOreName);
        ...
        return new ShapedOreRecipe(
            resourceHandler.getMainItemStack(recipe.getRecipeOutput()),
            RecipeHelper.rawShapeToShape(newInputs.toArray()));
    }

    @Override
    @Nonnull
    public ShapelessOreRecipe getNewShapelessRecipe(@Nonnull IRecipe recipe,
        @Nonnull final ResourceHandler resourceHandler, @Nonnull final UniOreDictionary uniOreDictionary) {
        final List<Object> newInputs = new ArrayList<>();
        for (final Object recipeInput : ((AdvShapelessRecipe) recipe).input) {
            if (recipeInput instanceof List) {
    ...
    @Override
    @Nonnull
    public ShapelessOreRecipe getNewShapelessFromShapedRecipe(@Nonnull IRecipe recipe,
        @Nonnull final ResourceHandler resourceHandler, @Nonnull final UniOreDictionary uniOreDictionary) {
        final List<Object> newInputs = new ArrayList<>();
        for (final Object recipeInput : ((AdvRecipe) recipe).input) {
            if (recipeInput instanceof IRecipeInput) {
                ...
                if (bufferOreName != null) newInputs.add(bufferOreName);
                    ...
                    if (obj instanceof ItemStack) newInputs.add(resourceHandler.getMainItemStack((ItemStack) obj));
                }
            }
        }
        return new ShapelessOreRecipe(resourceHandler.getMainItemStack(recipe.getRecipeOutput()), newInputs.toArray());
