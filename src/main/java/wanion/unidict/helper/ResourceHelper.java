...
public final class ResourceHelper implements IDependence {
    ...
    public long get(final Object thing) {
        return (thing instanceof ItemStack) ? stackKindMap.get(MetaItem.get((ItemStack) thing))
            : (thing instanceof List) ? oreKindMap.get(thing) : 0;
    }

    public void prepare(long kind) {
        if (kind == 0) return;
        for (final Resource resource : uniDictAPI.getResources(kind)) {
            final List<ItemStack> entries = resource.getChild(kind)
                .getEntries();
