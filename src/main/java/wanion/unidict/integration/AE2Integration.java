...
final class AE2Integration extends AbstractIntegrationThread {
    ...
    private void fixGrindStoneRecipes() {
        final List<IGrinderEntry> grindStoneRecipeList = AEApi.instance()
            .registries()
            .grinder()
            .getRecipes();
        final TIntSet uniques = new TIntHashSet(grindStoneRecipeList.size(), 1);
        for (final Iterator<IGrinderEntry> grindStoneRecipeIterator = grindStoneRecipeList
            .iterator(); grindStoneRecipeIterator.hasNext();) {
            ...
            if (!uniques.contains(recipeId)) {
                ...
                if (grindStoneRecipe.getOptionalOutput() != null) grindStoneRecipe.setOptionalOutput(
                    resourceHandler.getMainItemStack(grindStoneRecipe.getOptionalOutput()),
                    grindStoneRecipe.getOptionalChance());
                if (grindStoneRecipe.getSecondOptionalOutput() != null) grindStoneRecipe.setSecondOptionalOutput(
                    resourceHandler.getMainItemStack(grindStoneRecipe.getSecondOptionalOutput()),
                    grindStoneRecipe.getSecondOptionalChance());
