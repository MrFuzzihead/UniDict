...
final class FurnaceIntegration extends AbstractIntegrationThread {
    ...
    private void optimizeFurnaceRecipes() {
        if (!Config.inputReplacement)
            for (final Map.Entry<ItemStack, ItemStack> furnaceRecipe : (Set<Map.Entry<ItemStack, ItemStack>>) FurnaceRecipes
                .smelting()
                .getSmeltingList()
                .entrySet()) furnaceRecipe.setValue(resourceHandler.getMainItemStack(furnaceRecipe.getValue()));
        else {
            final Map<UniResourceContainer, TLongSet> containerKindMap = new IdentityHashMap<>();
            final Map<ItemStack, ItemStack> furnaceRecipes = FurnaceRecipes.smelting()
                .getSmeltingList();
            final Map<ItemStack, ItemStack> newRecipes = new HashMap<>();
            for (final Iterator<Map.Entry<ItemStack, ItemStack>> furnaceRecipeIterator = furnaceRecipes.entrySet()
                .iterator(); furnaceRecipeIterator.hasNext();) {
                ...
                if (!kindSet.contains(kind)) {
                    kindSet.add(kind);
                    newRecipes.put(
                        inputContainer.getMainEntry(furnaceRecipe.getKey().stackSize),
                        outputContainer.getMainEntry(furnaceRecipe.getValue().stackSize));
                }
