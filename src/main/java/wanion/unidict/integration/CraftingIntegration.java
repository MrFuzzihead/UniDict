...
final class CraftingIntegration extends AbstractIntegrationThread {
    ...
    private final UniOreDictionary uniOreDictionary = UniDict.getDependencies()
        .get(UniOreDictionary.class);
    ...
    CraftingIntegration() {
        ...
        if (Config.ic2) researcherList.add(new IC2RecipeResearcher());
        researcherList.forEach(researcher -> {
            researcher.getShapedRecipeClasses()
                .forEach(shapedRecipeClass -> shapedResearcherMap.put(shapedRecipeClass, researcher));
            researcher.getShapelessRecipeClasses()
                .forEach(shapelessRecipeClass -> shapelessResearcherMap.put(shapelessRecipeClass, researcher));
        });
    }
    ...
    private void doTheResearch() {
        ...
        for (final Iterator<IRecipe> recipeIterator = recipes.iterator(); recipeIterator.hasNext();) {
            ...
            if ((bufferRecipe = recipeIterator.next()) == null
                || (bufferContainer = resourceHandler.getContainer(bufferRecipe.getRecipeOutput())) == null
                || !(shapedResearcherMap.containsKey(bufferRecipe.getClass())
                    || (isShapeless = shapelessResearcherMap.containsKey(bufferRecipe.getClass()))))
                continue;
            final int recipeKey = !isShapeless ? shapedResearcherMap.get(bufferRecipe.getClass())
                .getShapedRecipeKey(bufferRecipe, resourceHandler)
                : shapelessResearcherMap.get(bufferRecipe.getClass())
                    .getShapelessRecipeKey(bufferRecipe, resourceHandler);
            if (recipeKey != 0 && bufferRecipe.getRecipeSize() > 0) {
                ...
                else evenSmarterRecipeMap.get(recipeKey)
                    .add(bufferRecipe);
            }
            recipeIterator.remove();
        }
        final TLongObjectMap<RecipeComparator> kindSpecificRecipeComparatorMap = Config.enableSpecificKindSort
            ? new TLongObjectHashMap<>()
            : null;
        final RecipeComparator normalRecipeComparator = !Config.enableSpecificKindSort
            ? new RecipeComparator(Util.itemStackComparatorByModName)
            : null;
        smartRecipeMap.forEach((container, evenSmartRecipeMap) -> evenSmartRecipeMap.forEachValue(recipeList -> {
            ...
            if (kindSpecificRecipeComparatorMap != null) {
                ...
                else kindSpecificRecipeComparatorMap
                    .put(container.kind, (recipeComparator = new RecipeComparator(container.getComparator())));
            } else recipeComparator = normalRecipeComparator;
            ...
            final IRecipeResearcher<? extends IRecipe, ? extends IRecipe> recipeResearcher = !isShapeless
                ? shapedResearcherMap.get(recipe.getClass())
                : shapelessResearcherMap.get(recipe.getClass());
            if (recipe.getRecipeSize() == 9) recipes.add(
                isShapeless
                    ? recipeResearcher.getNewShapedFromShapelessRecipe(recipe, resourceHandler, uniOreDictionary)
                    : recipeResearcher.getNewShapedRecipe(recipe, resourceHandler, uniOreDictionary));
            else if (recipe.getRecipeSize() == 1) recipes.add(
                isShapeless ? recipeResearcher.getNewShapelessRecipe(recipe, resourceHandler, uniOreDictionary)
                    : recipeResearcher.getNewShapelessFromShapedRecipe(recipe, resourceHandler, uniOreDictionary));
            else recipes.add(
                isShapeless ? recipeResearcher.getNewShapelessRecipe(recipe, resourceHandler, uniOreDictionary)
                    : recipeResearcher.getNewShapedRecipe(recipe, resourceHandler, uniOreDictionary));
            return true;
