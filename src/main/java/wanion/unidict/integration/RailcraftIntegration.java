...
final class RailcraftIntegration extends AbstractIntegrationThread {
    ...
    private final List<BlastFurnaceCraftingManager.BlastFurnaceRecipe> blastFurnaceRecipes = Util
        .getField(BlastFurnaceCraftingManager.class, "recipes", BlastFurnaceCraftingManager.getInstance(), List.class);
    ...
    private void fixBlastFurnaceRecipes() {
        if (blastFurnaceRecipes == null) return;
        final List<BlastFurnaceCraftingManager.BlastFurnaceRecipe> newRecipes = new FixedSizeList<>(
            blastFurnaceRecipes.size());
        for (final Iterator<BlastFurnaceCraftingManager.BlastFurnaceRecipe> blastFurnaceRecipeIterator = blastFurnaceRecipes
            .iterator(); blastFurnaceRecipeIterator.hasNext();) {
            ...
            newRecipes.add(
                new BlastFurnaceCraftingManager.BlastFurnaceRecipe(
                    recipe.getInput(),
                    recipe.matchDamage(),
                    recipe.matchNBT(),
                    recipe.getCookTime(),
