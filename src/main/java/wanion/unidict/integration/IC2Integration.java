...
final class IC2Integration extends AbstractIntegrationThread {
    ...
    private void fixMachinesOutputs(Map<IRecipeInput, RecipeOutput> recipes) {
        for (final Map.Entry<IRecipeInput, RecipeOutput> recipe : recipes.entrySet()) recipe.setValue(
            new RecipeOutput(
                recipe.getValue().metadata,
                resourceHandler.getMainItemStackList(recipe.getValue().items)));
    }
}
