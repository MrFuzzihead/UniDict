...
final class EnderIOIntegration extends AbstractIntegrationThread {
    ...
    public String call() {
        ...
        return threadName
            + "Some inanimate objects appear to have used ender pearls. They all disappeared, how this is possible?";
    }

    private void fixOreDictPreferences() {
        final Map preferences = Util
            .getField(OreDictionaryPreferences.class, "preferences", OreDictionaryPreferences.instance, Map.class);
        if (preferences != null) preferences.clear();
    }

    private void fixAlloySmelterRecipes() {
        final List<IManyToOneRecipe> alloySmelterRecipes = AlloyRecipeManager.getInstance()
            .getRecipes();
        final List<IManyToOneRecipe> newAlloySmelterRecipes = new FixedSizeList<>(alloySmelterRecipes.size());
        for (final Iterator<IManyToOneRecipe> alloySmelterRecipesIterator = alloySmelterRecipes
            .iterator(); alloySmelterRecipesIterator.hasNext();) {
            ...
            newAlloySmelterRecipes.add(
                new BasicManyToOneRecipe(
                    new Recipe(
                        new RecipeOutput(correctOutput, recipeOutput.getChance(), recipeOutput.getExperiance()),
                        alloySmelterRecipe.getEnergyRequired(),
                        alloySmelterRecipe.getBonusType(),
                        alloySmelterRecipe.getInputs())));
    ...
    private void fixSagMillRecipes() {
        final List<Recipe> sagMillRecipes = CrusherRecipeManager.getInstance()
            .getRecipes();
        final List<Recipe> newSagMillRecipes = new FixedSizeList<>(sagMillRecipes.size());
        newSagMillRecipes.addAll(
            sagMillRecipes.stream()
                .map(this::sagMillRecipe)
    ...
    private Recipe sagMillRecipe(@Nonnull final Recipe sagMillRecipe) {
        ...
        for (int i = 0; i < outputSize; i++) {
            ...
            output[i] = new RecipeOutput(
                resourceHandler.getMainItemStack(oldOutput.getOutput()),
                oldOutput.getChance(),
                oldOutput.getExperiance());
        }
        return new Recipe(
            sagMillRecipe.getInputs(),
            output,
            sagMillRecipe.getEnergyRequired(),
            sagMillRecipe.getBonusType());
    }
}
