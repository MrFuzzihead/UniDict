...
final class IEIntegration extends AbstractIntegrationThread {
    ...
    private void fixArcFurnaceRecipes() {
        ...
        final List<ArcFurnaceRecipe> correctRecipes = new ArrayList<>(
            new Double(arcFurnaceRecipes.size() * 1.3).intValue());
        for (final Iterator arcFurnaceRecipeIterator = arcFurnaceRecipes.iterator(); arcFurnaceRecipeIterator
            .hasNext();) {
    ...
    private void fixBlastFurnaceRecipes() {
        final List<BlastFurnaceRecipe> correctRecipes = new ArrayList<>(
            new Double(BlastFurnaceRecipe.recipeList.size() * 1.3).intValue());
        correctRecipes.addAll(
            BlastFurnaceRecipe.recipeList.stream()
                .map(
                    blastFurnaceRecipe -> new BlastFurnaceRecipe(
                        resourceHandler.getMainItemStack(blastFurnaceRecipe.output),
                        blastFurnaceRecipe.input,
                        blastFurnaceRecipe.time,
                        blastFurnaceRecipe.slag))
    ...
    private void fixCrusherRecipes() {
        ...
        for (final Iterator<CrusherRecipe> crusherRecipesIterator = crusherRecipes.iterator(); crusherRecipesIterator
            .hasNext();) {
            ...
            if (!uniques.contains(recipeId)) {
                ...
                else correctRecipes.add(
                    new UniCrusherRecipe(
                        correctOutput,
                        crusherRecipe.input,
                        crusherRecipe.energy,
                        resourceHandler.getMainItemStacks(crusherRecipe.secondaryOutput),
                        crusherRecipe.secondaryChance));
    ...
    private void fixMetalPressRecipes() {
        ...
        for (final Iterator<MetalPressRecipe> metalPressRecipesIterator = metalPressRecipes.values()
            ...
            if (!uniques.contains(id)) {
                correctRecipes.put(
                    metalPressRecipe.mold,
                    new MetalPressRecipe(
                        output,
                        metalPressRecipe.input,
                        metalPressRecipe.mold.stack,
                        metalPressRecipe.energy));
                uniques.add(id);
            }
    ...
    private static final class UniCrusherRecipe extends CrusherRecipe {
        ...
        private UniCrusherRecipe(ItemStack output, Object input, int energy, ItemStack[] secondaryOutputs,
            float[] secondaryChances) {
