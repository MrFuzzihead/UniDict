...
public final class FurnaceUniHelper implements IDependence {
    ...
    private final Map<ItemStack, ItemStack> furnaceRecipes = FurnaceRecipes.smelting()
        .getSmeltingList();
    ...
    private void populateSmartFurnaceRecipes() {
        ...
        for (ItemStack furnaceInputStack : furnaceRecipes.keySet()) {
            if ((hash = MetaItem.get(furnaceInputStack)) != 0) {
                final List<ItemStack> furnaceInputStacks = (!smartFurnaceRecipes.containsKey(hash))
                    ? new ArrayList<ItemStack>()
                    : smartFurnaceRecipes.get(hash);
