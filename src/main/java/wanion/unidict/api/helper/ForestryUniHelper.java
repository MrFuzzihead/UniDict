...
public final class ForestryUniHelper {
    ...
    private static final Set<ICarpenterRecipe> carpenterRecipes = Util
    ...
    public static void createCrates(@Nonnull final String kindName) {
        ...
        for (final Resource resource : UniDict.getResourceHandler().resources) {
            ...
            if (container != null) crates.put(container, new ItemCrated(container.getMainEntry(), true));
        }
    ...
    public static void registerCratesAndCreateRecipes(@Nonnull final Map<UniResourceContainer, ItemCrated> crates) {
        if (carpenterRecipes == null) return;
        for (final UniResourceContainer container : crates.keySet()) {
            ...
            GameRegistry.registerItem(
                crate,
                crate.getUnlocalizedName()
                    .replaceFirst("^item\\.", "crated"));
            ...
            carpenterRecipes.add(
                new CarpenterRecipe(
                    ...
                    null,
                    new ShapedRecipeCustom(new ItemStack(crate), "III", "III", "III", 'I', container.name)));
            carpenterRecipes.add(
                new CarpenterRecipe(
                    5,
                    null,
                    null,
                    new ShapedRecipeCustom(container.getMainEntry(9), "C  ", "   ", "   ", 'C', crate.getItemStack())));
        }
    }
}
