...
public final class ModuleHandler {
    ...
    public void startModules(final FMLStateEvent event) {
        ...
        if (modules.isEmpty() || loadStage == null) return;
        modules.entrySet()
            .forEach(e -> {
                final AbstractModule module = e.getKey();
                if (e.getValue() == null) e.setValue(module.manager);
                final AbstractModule.Manager manager = e.getValue();
                if (manager.isEmpty()) module.init();
                if (!manager.isEmpty(loadStage)) module.start(loadStage, manager);
            });
    }
    ...
    public void addModule(final AbstractModule module) {
        ...
        if (moduleClass == null || moduleClasses.contains(moduleClass)) return;
