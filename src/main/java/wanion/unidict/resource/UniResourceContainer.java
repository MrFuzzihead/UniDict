...
public final class UniResourceContainer {
    ...
    boolean updateEntries() {
        ...
        if (sort && initialSize != entries.size()) sort();
        ...
        if (sort) {
            ...
            if (autoHideInNEI) removeBadEntriesFromNEI();
            if (keepOneEntry) keepOneEntry();
        }
        return updated = true;
    }
    ...
    private void keepOneEntry() {
        ...
        if (!keepOneEntryBlackSet.isEmpty()) {
            for (Iterator<ItemStack> keepOneEntryIterator = entries.subList(1, entries.size())
                .iterator(); keepOneEntryIterator.hasNext();)
                if (!keepOneEntryBlackSet.contains(keepOneEntryIterator.next())) keepOneEntryIterator.remove();
        } else entries.subList(1, entries.size())
            .clear();
    }
    ...
    public Comparator<ItemStack> getComparator() {
        return enableSpecificKindSort ? SpecificKindItemStackComparator.getComparatorFor(kind)
